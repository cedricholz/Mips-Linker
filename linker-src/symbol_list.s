...
#		 $a0 0($a0)  4($a0)  8($a0)
##Symbol looks like |ADDRESS| NAME |ADDRESS| NAME |ADDRESS| NAME |/0
#		      $a0[0]  $a0[1] $a0[2]
...
loopdeloop:
	beqz $a0 symbol_not_founded # Check $a0 is not null
	move $s0 $a0     # Store a0 into s0
	lw $a0 4($s0)    # load the name into a0
	jal streq        # Check if its the word we're looking for
	move $a1 $s1     # Return a1 back to original state
	move $t0 $s0	 # Save symbol into t0
	beqz $v0 symbol_founded #if v0 is 0 then we've found it
	lw $a0 8($s0)    #Go to next symbol
	j loopdeloop

symbol_founded:
	lw $v0, 0($t0)     # Return the address
	j finishit

symbol_not_founded:
	li $v0, -1         # Return -1 if symbol not found
	j finishit
